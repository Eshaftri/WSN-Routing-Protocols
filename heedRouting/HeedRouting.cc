...
#include "HeedRouting.h"

Define_Module(HeedRouting);

void HeedRouting::startup()
{	
...
void HeedRouting::fromApplicationLayer(cPacket *pkt, const char *destination)
{	
	if(!isSink)
	{
		string dst(destination);
		HeedRoutingPacket *netPacket = new HeedRoutingPacket("Heed routing data packet", NETWORK_LAYER_PACKET);
		netPacket->setHeedRoutingPacketKind(HEED_ROUTING_DATA_PACKET);
...
void HeedRouting::fromMacLayer(cPacket *pkt, int macAddress, double rssi, double lqi){
	HeedRoutingPacket *netPacket = dynamic_cast <HeedRoutingPacket*>(pkt);

	if (!netPacket)
		return;

	switch (netPacket->getHeedRoutingPacketKind()) {

		case HEED_NEIGHBOURS_DISCOVERY_PACKET:{
		...
		case HEED_ROUTING_ADV_PACKET:{
			if(!isSink){	
				trace() << "Nodem " << self << " CH Message of node " << netPacket->getSource() ;	
				CHInfo rec;
...
void HeedRouting::timerFiredCallback(int index)
{
	timer = uniform(0,0.5);
	switch (index) {
		...
		/////////////////////////////////////////////////
			...
			setTimer(INITIALIZE, roundLength);
			break;
		}

		case SEND_ADV:{	
			if (neighboursMembers.size()!=0){
				HeedRoutingPacket *crtlPkt = new HeedRoutingPacket("ClusterHead Announcement Packet", NETWORK_LAYER_PACKET);
				crtlPkt->setByteLength(advPacketSize);
				crtlPkt->setHeedRoutingPacketKind(HEED_ROUTING_ADV_PACKET);
		...
		case JOIN_CH:{
			if(Sfinal.size()!=0){	
				Sfinal.sort(cpCst);
				HeedRoutingPacket *crtlPkt = new HeedRoutingPacket("ClusterMember Join Packet", NETWORK_LAYER_PACKET);
				crtlPkt->setHeedRoutingPacketKind(HEED_ROUTING_JOIN_PACKET);
		...
		case MAKE_TDMA:{
			if (clusterMembers.size()!=0){
				HeedRoutingPacket *crtlPkt = new HeedRoutingPacket("ClusterHead TDMA Packet", NETWORK_LAYER_PACKET);
				crtlPkt->setByteLength(tdmaPacketSize);
				crtlPkt->setHeedRoutingPacketKind(HEED_ROUTING_TDMA_PACKET);
				crtlPkt->setSource(SELF_NETWORK_ADDRESS);
				crtlPkt->setDestination(BROADCAST_NETWORK_ADDRESS);
...
void HeedRouting::sendAggregate()
{
	...
	{	
		...
		HeedRoutingPacket *aggrPacket = new HeedRoutingPacket("ClusterHead Aggredated Packet", NETWORK_LAYER_PACKET);
		aggrPacket->setByteLength(dataPacketSize+4);
		aggrPacket->setHeedRoutingPacketKind(HEED_ROUTING_DATA_PACKET);
...
void HeedRouting::processBufferedPacket()
{
...
void HeedRouting::readXMLparams()
{
...
void HeedRouting::neighboursDiscovery()
{
	HeedRoutingPacket *crtlPkt = new HeedRoutingPacket("Find Neighbours Packet", NETWORK_LAYER_PACKET);
		    crtlPkt->setByteLength(advPacketSize);
		    crtlPkt->setHeedRoutingPacketKind(HEED_NEIGHBOURS_DISCOVERY_PACKET);
		    crtlPkt->setSource(SELF_NETWORK_ADDRESS);
		    crtlPkt->setDestination(BROADCAST_NETWORK_ADDRESS);
		    toMacLayer(crtlPkt, BROADCAST_MAC_ADDRESS);
		   // trace() << "Nodeo " << self << " Sent Find Neighbours Beacon";		    	
}
double HeedRouting::getCHprob(void)
{
