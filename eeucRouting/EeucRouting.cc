...
{	
	...
	BSDistance = sqrt(pow((xlocal-xBS),2) + pow((ylocal-yBS),2));			//trovo la mia distanza dalla BS
...
void EeucRouting::fromApplicationLayer(cPacket *pkt, const char *destination){	
	if(!isSink)
	{
		string dst(destination);
		EeucRoutingPacket *netPacket = new EeucRoutingPacket("Eeuc routing data packet", NETWORK_LAYER_PACKET);
		netPacket->setEeucRoutingPacketKind(EEUC_ROUTING_DATA_PACKET);
...
void EeucRouting::fromMacLayer(cPacket *pkt, int macAddress, double rssi, double lqi){
	EeucRoutingPacket *netPacket = dynamic_cast <EeucRoutingPacket*>(pkt);
	...
	switch (netPacket->getEeucRoutingPacketKind()) {
		...
		case EEUC_ROUTING_COMPETECH_PACKET:{		
			if (newTentativeCH){				
				CHCompete comp;
		...
		case EEUC_ROUTING_FINALHEAD_PACKET:{
		...
		case EEUC_ROUTING_QUITELECTION_PACKET:{
		...
		case EEUC_ROUTING_ADJACENTCH_PACKET:{
			if(isCH){
		...
		case EEUC_ROUTING_AGGREGATEDADJACENT_PACKET:{
			string dst(netPacket->getDestination());
			if ((isCH) && (dst.compare(SELF_NETWORK_ADDRESS) == 0)){
				...
				if (BSDistance<TD_Max || CHNeighbors.size()==0){					
					...
					EeucRoutingPacket *aggrPacket = new EeucRoutingPacket("ClusterHead Aggredated Packet", NETWORK_LAYER_PACKET);
					aggrPacket->setByteLength(dataPacketSize+4);
					aggrPacket->setEeucRoutingPacketKind(EEUC_ROUTING_DATA_PACKET);
					...
					if (CHNeighbors.size()>1){
						...
						CHNeighbors.sort(cmpBSDistance);	
					...
					EeucRoutingPacket *aggrPacket = new EeucRoutingPacket("ClusterHead Aggredated Packet", NETWORK_LAYER_PACKET);
					aggrPacket->setByteLength(dataPacketSize+4);
					aggrPacket->setEeucRoutingPacketKind(EEUC_ROUTING_AGGREGATEDADJACENT_PACKET);
					aggrPacket->setSource(SELF_NETWORK_ADDRESS);
	...
	switch (index) {
		...
		case SEND_ADV:{	
			...
			if (!isCH){
				...
				EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("ClusterHead Announcement Packet", NETWORK_LAYER_PACKET);
				crtlPkt->setByteLength(advPacketSize);
				crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_ADV_PACKET);
		...
		case JOIN_CH:{
			...
			if(CHcandidates.size()!=0){	
				...
				CHcandidates.sort(eeCmpRssi);
				EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("ClusterMember Join Packet", NETWORK_LAYER_PACKET);
				crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_JOIN_PACKET);
		...
		case MAKE_TDMA:{
			if (isCH){
				...
				if (clusterMembers.size()!=0){
					EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("ClusterHead TDMA Packet", NETWORK_LAYER_PACKET);
					crtlPkt->setByteLength(tdmaPacketSize);
					crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_TDMA_PACKET);
					crtlPkt->setSource(SELF_NETWORK_ADDRESS);
					crtlPkt->setDestination(BROADCAST_NETWORK_ADDRESS);
		...
		case START_COMPETITION: {			
			...
			EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("Tentative Cluster Initializing Message", NETWORK_LAYER_PACKET);
			crtlPkt->setByteLength(advPacketSize);								//viene settata la dimensione del pacchetto, gi? specificata 
			crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_COMPETECH_PACKET);	//setto il tipo di EEUC packet; in EeucRoutingpacket.h
		...
		case FINISH_COMPETITION:{
			if (newTentativeCH){
				...
				if(lessSpentEnergy){
					...
					EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("Final Head Packet", NETWORK_LAYER_PACKET);
	
					crtlPkt->setByteLength(advPacketSize);
					crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_FINALHEAD_PACKET);
					crtlPkt->setSource(SELF_NETWORK_ADDRESS);
					crtlPkt->setDestination(BROADCAST_NETWORK_ADDRESS);
					toMacLayer(crtlPkt, BROADCAST_MAC_ADDRESS);
					isCH = true;
		...
		case SEND_QUIT:{
			...
			EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("Tentative Cluster Head Stopping Election", NETWORK_LAYER_PACKET);

			crtlPkt->setByteLength(advPacketSize);
			crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_QUITELECTION_PACKET);
			crtlPkt->setSource(SELF_NETWORK_ADDRESS);
			crtlPkt->setDestination(BROADCAST_NETWORK_ADDRESS);
		...
		case ADJACENT_CH_ADVERTISEMENT:{
			if (isCH){
				...
				EeucRoutingPacket *crtlPkt = new EeucRoutingPacket("Adjacent Cluster Head Advertisement", NETWORK_LAYER_PACKET);

				crtlPkt->setByteLength(advPacketSize);									//viene settata la dimensione del pacchetto, gi? specificata nel .ned file
				crtlPkt->setEeucRoutingPacketKind(EEUC_ROUTING_ADJACENTCH_PACKET);		//setto il tipo di EEUC packet; in EeucRoutingpacket.h
				crtlPkt->setSource(SELF_NETWORK_ADDRESS);							//setto il nodo sorgente; routingpacket.h
...
void EeucRouting::readXMLparams(){
...
bool eeCmpRssi(CHInfo a, CHInfo b){
	return (a.rssi > b.rssi);
}
bool cmpBSDistance(CHAdjacent a, CHAdjacent b){
